<!-- Source: https://ts.sdk.modelcontextprotocol.io/v2/get-started/first-server.html -->

# Build your first server [​](#build-your-first-server)
...
## Serve it over stdio [​](#serve-it-over-stdio)
...
Your server speaks stdio because a host launches it as a local process and owns its lifetime. To host one endpoint that many clients connect to, serve the same `createServer` factory over [HTTP](./../serving/http.html) instead.

Next on this path, [Plug into a real host](./real-host.html) registers this server in VS Code, Claude Code, and Cursor; [Tools](./../servers/tools.html) goes deeper on what a tool can return.
