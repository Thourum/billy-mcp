...
Any MCP host that launches this program lists and calls `get-forecast`; the SDK validates every call against that `z.object(...)` schema before your handler runs. [Build a server](./get-started/first-server.html) installs the packages and runs it end to end.

## Pick a path [​](#pick-a-path)

*   Expose your API or data to AI applications → **[Build a server](./get-started/first-server.html)**
*   Build an application that talks to MCP servers → **[Build a client](./get-started/first-client.html)**
*   Coming from v1 (`@modelcontextprotocol/sdk`) → **[Upgrade](./migration/)**
*   Drop MCP into the app you already run → **[Express](./serving/express.html)** · **[Hono](./serving/hono.html)** · **[Fastify](./serving/fastify.html)** · **[Workers](./serving/web-standard.html)**

For exact signatures, go to the [API reference](/v2/api/).
